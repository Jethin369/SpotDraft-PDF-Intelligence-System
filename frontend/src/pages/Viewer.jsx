...
export default function Viewer() {
    ...
    const fetchData = async () => {
      try {
        ...
        const endpoint = isShared 
          ? `https://spotdraft-pdf-v3.onrender.com/share/${linkId}` 
          : `https://spotdraft-pdf-v3.onrender.com/documents/${docId}`;
        ...
        // Fetch comments
        const commentsRes = await fetch(`https://spotdraft-pdf-v3.onrender.com/comments/${currentId}`);
        if (commentsRes.ok) {
  ...
  const handleChat = async () => {
    ...
    try {
      const res = await fetch('https://spotdraft-pdf-v3.onrender.com/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          document_id: currentId, 
  ...
  const handleShare = async () => {
    try {
      const res = await fetch(`https://spotdraft-pdf-v3.onrender.com/share/${docId}`, { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
